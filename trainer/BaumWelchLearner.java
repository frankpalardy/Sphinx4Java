...
public class BaumWelchLearner implements Learner, Configurable {
    ...
    public void newProperties(PropertySheet ps) throws PropertyException {
    	...
    	inputDataType = ps.getString(PROP_INPUT_TYPE, 
...
// since this comes from the configuration file dynamically the logic isn't needed anymore
	...
	public TrainerScore[] getScore() {
		...
		if (currentFeatureIndex >= 0) {
			...
			if (currentFeatureIndex == lastFeatureIndex - 1) {
				...
				if (Math.abs(totalLogScore - logScore) > Math
						.abs(totalLogScore)) {
	...
	private void backwardPass(TrainerScore[] score) {
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			// this makes no sense because it is setting the probability to zero
			// the only way this can improve is if the previous probability was higher
			// but that can't happen
			probCurrentFrame[indexNode] = LogMath.log10(0.5f);
	 ...
	 * initialTokenlist = new List; newtokenlist.add(token);
	 *  // Initial token is on a nonemitting state; no need to score; List
	 * newList = expandToEmittingStateList(initialTokenList){
	 * 
	 ...
	 * new FastActiveList();
	 *  // At this point we have only emitting states. We score // and prune
	 * them ActiveList emittingStateList = new FastActiveList(); //
	 ...
	 * transition.posterior = alpha*transition*beta / // totalprobability; 
	 * double localtransitionbetascore = logtransition + logbeta + transition.destination.logscore; 
	 * double transition.posterior = localtransitionbetascore + logalpha - logtotalprobability;
	...
	/*
	 * private void expandStateList(ActiveList stateList, ActiveList
	 * nextActiveList) { while (stateList.hasMoreTokens()) { Token token =
	 * emittingStateList.getNextToken();
	 *  // First get list of links to possible future states List successorList =
	 * getSuccessors(token); while (successorList.hasMoreEntries()) {
	 * UtteranceGraphEdge edge = successorList.getNextEntry();
	 *  // create a token for the future state, if its not // already in active
	 * list; The active list will check // for the key "edge.destination()" in
	 * both of its // lists if (nextActiveList.hasState(edge.destination())) {
	 * Token newToken = nextActiveList.getTokenForState(edge.destination()); }
	 * else { Token newToken = new Token(edge.destination()); }
	 *  // create a link between current state and future state TrainerLink
	 * newlink = new TrainerLink(edge, token, newToken); newlink.logScore =
	 * token.logScore + edge.transition.logprob();
	 *  // add link to the appropriate lists for source and // destination
	 * tokens token.addOutGoingLink(newlink);
	 * 
	 * newToken.addIncomingLink(newlink); newToken.alpha =
	 * logAdd(newToken.alpha, newlink.logScore);
	 *  // At this point, we have placed a new token in the // successor state,
	 * and linked the token at the // current state to the token at the
	 * non-emitting // states.
	 *  // Add token to appropriate active list nextActiveList.add(newToken); } } }
	 */
