...
public class BuildTranscriptHMM {
	 ...
	 */
	public BuildTranscriptHMM(Transcript transcript, AcousticModel acousticModel) {
		...
		// System.out.println("BuildTranscriptHMM needs work");
		contextDependentPhoneGraph = buildContextDependentPhonemeGraph(phonemeGraph);

		if (contextDependentPhoneGraph.validate() == false) {
			System.out.println("Context dependent graph not validated");
		}
	...
	private Graph buildWordGraph(Transcript transcript) {
		...
		// Make sure the dictionary is a TrainerDictionary before we cast assert
		...
		// numWords is the number of words in the transcript line
		...
		if (transcript.isExact()) {
			...
			Node silLoopBack = new Node(NodeType.SILENCE_WITH_LOOPBACK, "SIL");
			...
			for (transcript.startWordIterator(); transcript.hasMoreWords();) {
				...
				/* Add silence */
				silLoopBack = new Node(NodeType.SILENCE_WITH_LOOPBACK, "SIL");
	...
	private Graph buildPhonemeGraph(Graph wordGraph) {
		...
		// I don't know if it's a good idea to copy the word graph cause then
		// parts of it
		// are left over, such as dummys and silence
	...
	public Graph buildHMMGraph(Graph cdGraph) {
		...
		for (int i = 0; i < nodeArray.length; i++) {
			...
			Unit unit;
			// I had to change the id in the Unit and UnitManager to a String
			// for this to work
			if (node.getType().equals(NodeType.PHONE)) {
				// unit = acousticModel.lookupUnit(node.getID());
				// unit = Unit.getUnit(node.getID());
				unit = new Unit(node.getID(), false, 11);
				// unit = new Unit(node.getID(), false, i);
			} else if (node.getType().equals(NodeType.SILENCE_WITH_LOOPBACK)) {
				unit = new Unit(node.getID(), true, 1);
				// I made the silence with loopbacks all id "SIL" FP
				continue;
			} else {
				// it's a dummy node, and we don't care.
				continue;
			}
