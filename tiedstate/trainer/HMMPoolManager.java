...
public class HMMPoolManager implements Configurable {
	...
	private void accumulateMean(int senone, TrainerScore score) {
		if (senone == TrainerAcousticModel.ALL_MODELS) {
			...
			for (int i = 0; i < mix.length; i++) {
				...
				float prob = score.getComponentGamma()[0];
				// I had to change this cause there was only one of these FP
	...
	private void accumulateVariance(int senone, TrainerScore score) {
		if (senone == TrainerAcousticModel.ALL_MODELS) {
			...
			for (int i = 0; i < mix.length; i++) {
				...
				//this doesn't actually save the buffer so it's not doing anything
				buffer.accumulate(data, dprob);
			}
		}
	}
	...
	private void accumulateMixture(int senone, TrainerScore score) {
		...
		if (senone == TrainerAcousticModel.ALL_MODELS) {
			...
			for (int i = 0; i < mixw.length; i++) {
				//had to change this cause only one gamma frank
				//float prob = score.getComponentGamma()[i];
				float prob = score.getComponentGamma()[0];
				prob -= currentLogLikelihood;
	...
	private void accumulateStateTransition(int indexScore,
		...
		for (int i = 0; i < vector.length; i++) {
			...
			if (vector[i] != LogMath.getLogZero()) {
				...
				//since this buffer is not saved it basically vanishes
	 ...
	 */
	// this whole method doesn't do anything because there's no pointers in java
		...
		for (int i = 0; i < transitionsPool.size(); i++) {
			...
			for (int j = 0; j < matrix.length; j++) {
				...
				if (buffer.wasUsed()) {
					...
					//this actually works but stays in the buffer
					buffer.logNormalizeToSum(logMath);

					// I don't think this is needed. The vectors would copy just with =
					matrix[j] = copyVector(buffer.getValues(), matrix[j]);
					bufferArray[j] = buffer;
				}
			}
