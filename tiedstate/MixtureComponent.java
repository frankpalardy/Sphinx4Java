...
/**
 * Defines the set of shared elements for a GaussianMixture. Since these
 * elements are potentially shared by a number of
 * {@link GaussianMixture GaussianMixtures}, these elements should not be
 * written to. The GaussianMixture defines a single probability density function
...
public class MixtureComponent {
	 ...
	 *            transformation matrix for this pdf
	 * @param meanTransformationVector
	 *            transform vector for this PDF
	 * @param variance
	 *            the variance for this PDF
	 * @param varianceTransformationMatrix
	 *            var. transform matrix for this PDF
	 * @param varianceTransformationVector
	 *            var. transform vector for this PDF
	 * @param distFloor
	 *            the lowest score value
	 * @param varianceFloor
	 *            the lowest value for the variance
	 */
	public MixtureComponent(LogMath logMath, float[] mean,
			float[][] meanTransformationMatrix,
			float[] meanTransformationVector, float[] variance,
			float[][] varianceTransformationMatrix,
			float[] varianceTransformationVector, float distFloor,
			float varianceFloor) {
	...
	/**
	 * Calculate the score for this mixture against the given feature. We model
	 * the output distributions using a mixture of Gaussians, therefore the
	 * current implementation is simply the computation of a multi-dimensional
	 * Gaussian.
	 * 
	 * <p>
	 * <b>Normal(x) = exp{-0.5 * (x-m)' * inv(Var) * (x-m)} / {sqrt((2 * PI) ^
	 * N) * det(Var))}</b>
	 * </p>
	 * 
	 * where <b>x</b> and <b>m</b> are the incoming cepstra and mean vector
	 * respectivally, <b>Var</b> is the Covariance matrix, <b>det()</b> is the
	 * determinant of a matrix, <b>inv()</b> is its inverse, <b>exp</b> is the
	 * exponential operator, <b>x'</b> is the transposed vector of <b>x</b>
	 * and <b>N</b> is the dimension of the vectors <b>x</b> and <b>m</b>.
	 * 
	 * @param feature
	...
	public float getScore(Data feature) {
		...
		// for some reason the training wants a double 
		// need to fix this cause it stops the recognizer from working
	    //float[] data = ((FloatData) feature).getValues();
		 double[] data = ((DoubleData) feature).getValues();
		// float logVal = 0.0f;
		float logDval = 0.0f;
		...
		for (int i = 0; i < data.length; i++) {
			double meanT = meanTransformed[i];
			double logDiff = data[i] - meanT;
		...
		// the inverse log base doesn't seem right FP
...
*/
	/**
	 * Pre-compute factors for the Mahalanobis distance. Some of the Mahalanobis
	 * distance computation can be carried out in advance. Especifically, the
	 * factor containing only variance in the Gaussian can be computed in
	 * advance, keeping in mind that the the determinant of the covariance
	 * matrix, for the degenerate case of a mixture with independent components -
	 * only the diagonal elements are non-zero - is simply the product of the
	 * diagonal elements.
	 * 
	 * We're computing the expression:
	 * 
	 * <p>
	 * <b>{sqrt((2 * PI) ^ N) * det(Var))}</b>
	 * </p>
	 * 
	...
	public float precomputeDistance() {
		...
		for (int i = 0; i < variance.length; i++) {
			logPreComputedGaussianFactor += logMath
					.linearToLog(precisionTransformed[i] * -2);
		...
		// this is always coming out to the same thing be inverselog is always the same
		logPreComputedGaussianFactor = logMath.linearToLog(2.0 * Math.PI)
				* variance.length - logPreComputedGaussianFactor;
	...
	private void transformStats() {
		 ...
		 * </p>
		 * 
		 * where <b>M</b> and <b>m</b> are the mean vector after and before
		 * transformation, respectively, and <b>A</b> and <b>B</b> are the
		 * transformation matrix and vector, respectively.
		 */
		...
		/**
		 * We do analogously with the variance. In this case, we also invert the
		 * variance, and work with precision instead of variance.
		 */
