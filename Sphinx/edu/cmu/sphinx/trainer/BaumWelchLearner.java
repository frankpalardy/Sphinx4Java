 ...
 * data.
 */
public class BaumWelchLearner implements Learner, Configurable {

	private final static String PROP_PREFIX = "edu.cmu.sphinx.trainer.";

	/**
	 * Sphinx property that defines the name of the logmath from
	 * SimpleBeadthFirstManager
	 */
	...
	private static Logger logger = Logger
			.getLogger("edu.cmu.sphinx.trainer.BaumWelch");
	...
	// logMath = LogMath.getLogMath(context);this should be done in properties
	// initialize();
	// }
	/**
	...
	public void initialize() throws IOException {
		// since this comes from the configuration file dynamically the logic
	 ...
	 * 
	 * protected FrontEnd getFrontEnd() { String path = null; try { FrontEnd fe =
	 * null; Collection names = FrontEndFactory.getNames(props); // assert
	 * names.size() == 1; for (Iterator i = names.iterator(); i.hasNext();) {
	 * String feName = (String) i.next(); fe =
	 * FrontEndFactory.getFrontEnd(props, feName); } return fe; } catch
	 * (InstantiationException ie) { throw new Error("IE: Can't create front end " +
	 * path, ie); } }
	 */
	...
	private boolean getFeature() {
		try {
			...
			if (curFeature instanceof DataStartSignal) {
				// this is to get the next feature
				// curFeature = frontEnd.getData();
				curFeature = dataSource.getData();
				if (curFeature == null) {
					return false;
	...
	public void initializeComputation(Utterance utterance, UtteranceGraph graph)
			throws IOException {
		// initialize(); seems like this is already done
	...
	/**
	 * Prepares the learner for returning scores, one at a time. To do so, it
	 * performs the full forward pass, but returns the scores for the backward
	 * pass one feature frame at a time.
	 */
	private Object[] prepareScore() {
		...
		for (finalNode.startIncomingEdgeIterator(); finalNode
	...
	public TrainerScore[] getScore() {
		...
		// Object scores []= scoreArray.toArray();
		// while (currentFeatureIndex > 0) {
		currentFeatureIndex--;
		if (currentFeatureIndex >= 0) {
			float logScore = LogMath.getLogZero();
			score = (TrainerScore[]) scoreArray[currentFeatureIndex];
			// score = (TrainerScore[]) scores[currentFeatureIndex];
			// assert score.length == betas.length;
			score = backwardPass(score);
				...
				if (currentFeatureIndex == lastFeatureIndex - 1) {
					for (int i = 0; i < betas.length; i++) {
						for (int j = 0; j < score.length; j++) {
							((TrainerScore[]) scoreArray[i])[j].setGamma();
							logScore = logMath.addAsLinear(logScore,
									((TrainerScore[]) scoreArray[i])[j].getGamma());
							((TrainerScore[]) scoreArray[i])[0]
									.setScalingFactor(logMath.addAsLinear(
											((TrainerScore[]) scoreArray[i])[0]
													.getScalingFactor(), logScore));
						}
						for (int j = 0; j < ((TrainerScore[]) scoreArray[i]).length; j++) {
							((TrainerScore[]) scoreArray[i])[j]
									.setScalingFactor(((TrainerScore[]) scoreArray[i])[0]
											.getScalingFactor());
							// System.out.println("score=
							// "+logMath.logToLinear(((TrainerScore[])
							// scores[i])[0].getScalingFactor()));
						}
			...
			// System.out.println(logMath.logToLinear(logScore));
			totalLogScore = logMath.addAsLinear(totalLogScore, logScore);
			return score;
		} else {
			scoreArray = null;
			return null;
	...
	/**
	 * Computes the acoustic scores using the current Feature and a given node
	 * in the graph.
	 * 
	 * @param index
	 *            the graph index
	...
	private float calculateScores(int index) {
		...
		SenoneHMMState state = (SenoneHMMState) graph.getNode(index)
				.getObject();
		if ((state != null) && (state.isEmitting())) {// if was checking if
			// state was emitting
			// but then it didn't
			// set the component
			// gamma
	 ...
	 * 
	 * @param score
	 *            the objects transferring info to the buffers
	 */
	// it's not saving the scores because not passed back
	private TrainerScore[] forwardPass(TrainerScore[] score) {
		// Let's precompute the acoustic probabilities and create the
		// score object, one for each state
		for (int i = 0; i < graph.size(); i++) {
			// System.out.println((HMMState) graph.getNode(i).getObject());
			outputProbs[i] = calculateScores(i);
			// System.out.println(outputProbs[0]);
			score[i] = new TrainerScore(curFeature, outputProbs[i],
					(HMMState) graph.getNode(i).getObject(), componentScores);
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			probCurrentFrame[indexNode] = logMath.getLogOne();
			for (node.startIncomingEdgeIterator(); node.hasMoreIncomingEdges();) {
				...
				if (previousState != null) {
					...
					if ((!previousState.isEmitting())
						...
						System.out.println("problem with previous state");
					}
					if (!previousState.isEmitting()) {
						logTransitionProbability = 0.0f;
					} else {
						logTransitionProbability = hmm
								.getTransitionProbability(previousState
				...
				probCurrentFrame[indexNode] = logMath.addAsLinear(
			...
			// System.out.println("State= " + indexNode + " output= " +
			// outputProbs[indexNode]);
			score[indexNode].setAlpha(probCurrentFrame[indexNode]);
			totalAlpha = logMath.addAsLinear(totalAlpha,
					probCurrentFrame[indexNode]);
			// System.out.println(logMath.logToLinear(totalAlpha)+ " "+
			// logMath.logToLinear(probCurrentFrame[indexNode]));
		}
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			for (node.startIncomingEdgeIterator(); node.hasMoreIncomingEdges();) {
				...
				if (previousNode.isType("STATE")) {
					HMMState previousState = (HMMState) previousNode
							.getObject();
		...
		// for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
		// score[indexNode].setAlpha(score[indexNode].getAlpha() - totalAlpha);
		// }
		return score;
	}
	 ...
	 */
	private TrainerScore[] backwardPass(TrainerScore[] score) {
		...
		for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
			...
			for (node.startOutgoingEdgeIterator(); node.hasMoreOutgoingEdges();) {
				...
				if (nextState != null) {
					...
					if (nextState.getHMM() != hmm) {
						...
						logTransitionProbability = hmm
								.getTransitionProbability(state.getState(),
										nextState.getState());
					}
				} else {
				...
				// System.out.println(probCurrentFrame[indexNode]+ " "+
				// probNextFrame[indexNextNode] +" "+ logTransitionProbability
			...
			// System.out.println("State= " + indexNode + " beta= " +
			// probCurrentFrame[indexNode]);
			score[indexNode].setBeta(probCurrentFrame[indexNode]);
			totalBeta = logMath.addAsLinear(totalBeta,
					probCurrentFrame[indexNode]);
		}
		...
		for (int indexNode = graph.size() - 1; indexNode >= 0; indexNode--) {
			...
			for (node.startOutgoingEdgeIterator(); node.hasMoreOutgoingEdges();) {
				...
				if (nextNode.isType("STATE")) {
					...
					// assert ((nextState.isEmitting()) || (nextState ==
					// state));
					// In any case, the transition (at this point) is
					// assumed to be 1.0f, or 0.0f in log scale.
					logTransitionProbability = 0.0f;
					/*
					 * if (!nextState.isEmitting()) { logTransitionProbability =
					 * 0.0f; } else { logTransitionProbability =
		...
		// for (int indexNode = 0; indexNode < graph.size(); indexNode++) {
		// score[indexNode].setBeta(score[indexNode].getBeta() - totalBeta);
		// }
		return score;
	}

	/*
	 * Pseudo code: forward pass: token = maketoken(initialstate); List
	 * initialTokenlist = new List; newtokenlist.add(token); // Initial token is
	 * on a nonemitting state; no need to score; List newList =
	 * expandToEmittingStateList(initialTokenList){
	 * 
	 * while (morefeatures){ scoreTokenList(emittingTokenList,
	 * featurevector[timestamp]); pruneTokenList(emittingTokenList); List
	 * newList = expandToEmittingStateList(emittingTokenList){ timestamp++; } //
	 * Some logic to expand to a final nonemitting state (how)?
	 * expandToNonEmittingStates(emittingTokenList);
	 */

	/*
	 * private void forwardPass() { ActiveList activelist = new
	 * FastActiveList(createInitialToken()); AcousticScorer acousticScorer = new
	 * ThreadedAcousticScorer(); FeatureFrame featureFrame =
	 * frontEnd.getFeatureFrame(1, ""); Pruner pruner = new SimplePruner(); //
	 * Initialization code pushing initial state to emitting state here
	 * 
	 * while ((featureFrame.getFeatures() != null)) { ActiveList nextActiveList =
	 * new FastActiveList(); // At this point we have only emitting states. We
	 * score // and prune them ActiveList emittingStateList = new
	 * FastActiveList(); // activelist.getEmittingStateList();
	 * acousticScorer.calculateScores(emittingStateList.getTokens()); // The
	 * pruner must clear up references to pruned objects emittingStateList =
	 * pruner.prune( emittingStateList);
	 * 
	 * expandStateList(emittingStateList, nextActiveList);
	 * 
	 * while (nextActiveList.hasNonEmittingStates()){ //
	 * extractNonEmittingStateList will pull out the list // of nonemitting
	 * states completely from the // nextActiveList. At this point
	 * nextActiveList does // not have a list of nonemitting states and must //
	 * instantiate a new one. ActiveList nonEmittingStateList =
	 * nextActiveList.extractNonEmittingStateList(); nonEmittingStateList =
	 * pruner.prune(nonEmittingStateList); expandStateList(nonEmittingStateList,
	 * nextActiveList); } activeList = newActiveList; } }
	 */

	/*
	 * Pseudo code backward pass: state =
	 * finaltoken.state.wholelistofeverythingthatcouldbefinal; while
	 * (moreTokensAtCurrentTime) { Token token = nextToken(); State state =
	 * token.state; state.gamma = state.logalpha + state.logbeta -
	 * logtotalprobability;
	 * 
	 ...
	 * while token.hasMoreIncomingEdges() { Edge transition =
	 * token.nextIncomingEdge(); double logalpha = transition.source.alpha;
	 * double logbeta = transition.destination.beta; double logtransition =
	 * transition.transitionprob; transition.posterior = alpha*transition*beta /
	 * totalprobability; double localtransitionbetascore = logtransition +
	 * logbeta + transition.destination.logscore; double transition.posterior =
	 * localtransitionbetascore + logalpha - logtotalprobability;
	 ...
	 * localtransitionbetascore); /* private void expandStateList(ActiveList
	 * stateList, ActiveList nextActiveList) { while (stateList.hasMoreTokens()) {
	 * Token token = emittingStateList.getNextToken(); // First get list of
	 * links to possible future states List successorList =
	 * getSuccessors(token); while (successorList.hasMoreEntries()) {
	 * UtteranceGraphEdge edge = successorList.getNextEntry(); // create a token
	 * for the future state, if its not // already in active list; The active
	 * list will check // for the key "edge.destination()" in both of its //
	 * lists if (nextActiveList.hasState(edge.destination())) { Token newToken =
	 * nextActiveList.getTokenForState(edge.destination()); } else { Token
	 * newToken = new Token(edge.destination()); } // create a link between
	 * current state and future state TrainerLink newlink = new
	 * TrainerLink(edge, token, newToken); newlink.logScore = token.logScore +
	 * edge.transition.logprob(); // add link to the appropriate lists for
	 * source and // destination tokens token.addOutGoingLink(newlink);
	 * 
	 * newToken.addIncomingLink(newlink); newToken.alpha =
	 * logAdd(newToken.alpha, newlink.logScore); // At this point, we have
	 * placed a new token in the // successor state, and linked the token at the //
	 * current state to the token at the non-emitting // states. // Add token to
	 * appropriate active list nextActiveList.add(newToken); } } }
	 */

	/*
	 * private void expandToEmittingStateList(List tokenList){ List
	 * emittingTokenList = new List(); do { List nonEmittingTokenList = new
	 * List(); expandtokens(newtokenlist, emittingTokenList,
	 * nonemittingTokenList); while (nonEmittingTokenList.length() != 0); return
	 * emittingTokenList; } }
	 */

	/*
	 * private void expandtokens(List tokens, List nonEmittingStateList, List
	 * EmittingStateList){ while (moreTokens){ sucessorlist =
	 * SentenceHMM.gettransitions(nextToken()); while (moretransitions()){
	 * transition = successor; State destinationState = successor.state;
	 * newtoken = gettokenfromHash(destinationState, currenttimestamp);
	 * newtoken.logscore = Logadd(newtoken.logscore, token.logscore +
	 * transition.logscore); // Add transition to newtoken predecessor list? //
	 * Add transition to token sucessor list // Should we define a token "arc"
	 * for this. ?? if (state.isemitting) EmittingStateList.add(newtoken); else
