...
public class TrainerDictionary extends FullDictionary {
	 ...
	 */
	public TrainerDictionary() throws IllegalArgumentException, IOException {
	...
	public Graph getWordGraph(String word, boolean hasDummy) {
		...
		pronunciations = getWord(wordWithoutParentheses)
		...
		if (pronunciationID >= pronunciations.length) {
			System.out.println("Dictionary has only " + pronunciations.length
					+ " for word " + word);
			return null;
		}
		...
		for (int i = 0; i < units.length; i++) {
			...
			wordNode = new Node(NodeType.STATE, units[i].getName());
			if (prevNode == null) {
	...
	public String toString() {
		return "DEFAULT";//super.wordDictionaryFile.toString();
	}
}
