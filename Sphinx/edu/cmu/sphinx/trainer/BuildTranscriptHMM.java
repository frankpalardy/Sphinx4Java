...
public class BuildTranscriptHMM {
	...
	public BuildTranscriptHMM(Transcript transcript,
		...
		hmmGraph = buildCIHMMGraph(phonemeGraph);
	...
	private Graph buildCIHMMGraph(Graph graph) {
		...
		// now the initial and final nodes
		// the initial node should be added first but this works right now
		if (!hmmGraph.isNodeInGraph(graph.getInitialNode())) {
			// this is how utterance is getting in
			hmmGraph.addNode(graph.getInitialNode());
			hmmGraph.setInitialNode(graph.getInitialNode());
		} else {
			hmmGraph.setInitialNode(graph.getInitialNode());
		}
		if (!hmmGraph.isNodeInGraph(graph.getFinalNode())) {
			hmmGraph.addNode(graph.getFinalNode());
			hmmGraph.setFinalNode(graph.getFinalNode());
		} else {
			hmmGraph.setFinalNode(graph.getFinalNode());
		}
		return hmmGraph;
