...
public class Graphb {
	...
	/**
	 * Link two nodes. If the source or destination nodes are not in the graph,
	 * they are added to it. No check is performed to ensure that the nodes are
	 * linked to other nodes in the graph.
	 */
	 ...
	 *            the graph to insert
	 * @param node
	 *            the node that this graph will replace
	 */
	public void insertGraph(Graph graph, Node node) {
		...
		int index;
		for (graph.startNodeIterator(), index = nodePosition; graph
				.hasMoreNodes(); index++) {
	...
	/**
	 * Validate the graph. It checks out basics about the graph, such as whether
	 * all nodes have at least one incoming and outgoing edge, except for the
	 * initial and final.
	 * 
	...
	public boolean validate() {
		...
		for (startNodeIterator(); hasMoreNodes();) {
			...
			for (node.startIncomingEdgeIterator(); node.hasMoreIncomingEdges();) {
				Edge edge = node.nextIncomingEdge();
				passed &= edges.contains(edge);

				if (!edges.contains(edge)) {
					System.out.println("missing incoming edge");
				}
			}
			if (outgoing < 1) {
				if (!isFinalNode(node)) {
			...
			for (node.startOutgoingEdgeIterator(); node.hasMoreOutgoingEdges();) {
				...
				if (!edges.contains(edge)) {
					System.out.println("missing outgoing edge");
				}
			}
		}
