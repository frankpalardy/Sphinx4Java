...
public class Sphinx3Saver implements Saver, Configurable{
	...
	private void saveModelFiles(String modelName) throws FileNotFoundException,
		...
		System.out.println("Saving acoustic model: " + modelDef);
	 ...
	 *            the floored value
	 */
	private float[] nonZeroFloor(float[] data, float floor) {
	...
	private void saveHMMPool(boolean useCDUnits, OutputStream outputStream,
		...
		//	if (!unit.getContext().equals(Context.EMPTY_CONTEXT)){
				//I don't think I need this cause they go through the loop again later
		//		left = ((LeftRightContext)unit.getContext()).getLeftContext()[0].getName();
		//		right = ((LeftRightContext)unit.getContext()).getRightContext()[0].getName();
		//		pw.print(left  + " \t");
		//		pw.print(right  + " \t");
		//	} else {
				pw.print(left  + "  ");
