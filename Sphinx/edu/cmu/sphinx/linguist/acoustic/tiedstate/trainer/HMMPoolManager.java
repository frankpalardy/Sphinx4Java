 ...
 */
public class HMMPoolManager implements Configurable {
	/**
	...
	private Pool transitionsBufferPool;
	...
	private static Logger logger = Logger
			.getLogger("edu.cmu.sphinx.linguist.acoustic.HMMPoolManager");
	...
	/**
	 * Constructor for this pool manager. It gets the pointers to the pools from
	 * a loader.
	 * 
	 * @param loader
	 *            the loader protected HMMPoolManager(Loader loader,
	...
	public void init(Loader loader) {
		...
		transitionsPool = loader.getTransitionMatrixPool();
		senonePool = loader.getSenonePool();
		//logMixtureWeightFloor = logMath.linearToLog(loader.getMixtureWeightFloor());
		//logTransitionProbabilityFloor = 
		 //   logMath.linearToLog(loader.getTransitionProbabilityFloor());
		//createBuffers(); these are not needed because they are in resetBuffers
		//logLikelihood = 0.0f;
	}
	...
	protected void createBuffers() {
		...
		transitionsBufferPool = create2DPoolBuffer(transitionsPool, true);
	...
	private Pool create2DPoolBuffer(Pool pool, boolean isLog) {
		...
		for (int i = 0; i < pool.size(); i++) {
			float[][] element = (float[][]) pool.get(i);
			indexMap2.put(element, new Integer(i));
	 ...
	 *            the TrainerScore
	 */
	public void accumulate(int index, TrainerScore[] score) {
	 ...
	 *            the current index into the TrainerScore vector
	 * @param score
	 *            the TrainerScore for the current frame
	 * @param nextScore
	 *            the TrainerScore for the next time frame
	 */
	public void accumulate(int index, TrainerScore[] score,
		...
		// We should be doing this just once per utterance...
		  currentLogLikelihood = thisScore.getScalingFactor();
		...
		if (state == null) {
			...
			if (state.isEmitting()) {
				//updateLogLikelihood(score[index]);
	...
	private void accumulateMean(int senone, TrainerScore score) {
		if (senone == TrainerAcousticModel.ALL_MODELS) {
			...
			for (int i = 0; i < mix.length; i++) {
				...
				double[] feature = ((DoubleData) score.getData()).getValues();				
				//float[] feature = ((FloatData) score.getData()).getValues();
				double[] data = new double[feature.length];
				float prob = score.getGamma();
				prob -= currentLogLikelihood; 
 			 	//System.out.println(logMath.logToLinear(prob) + " "+ senone);
				double dprob = logMath.logToLinear(prob);
				for (int j = 0; j < data.length; j++) {
					data[j] = feature[j] * dprob;
				}
				buffer.accumulate(data, dprob,feature);
				//meansBufferPool.put(senone, buffer);
			}
		}
	}
		...
		if (senone == TrainerAcousticModel.ALL_MODELS) {
			...
			for (int i = 0; i < mix.length; i++) {
				...
				for (int j = 0; j < data.length; j++) {
					...
					data[j] *= data[j] * dprob;
					//System.out.println(data[j]+" "+dprob+" "+senone+" "+logMath.logToLinear(currentLogLikelihood));
				}
				buffer.accumulate(data, dprob, feature);
				//varianceBufferPool.put(i, buffer);
			}
		}
	}
	...
	private void accumulateMixture(int senone, TrainerScore score) {
		...
		if (senone == TrainerAcousticModel.ALL_MODELS) {
			...
			//mixtureWeightsBufferPool.put(senone, buffer);
		}
	}
	 ...
	 * 
	 * @param indexScore
	 *            the current index into the TrainerScore
	 * @param score
	 *            the score information
	 * @param nextScore
	 *            the score information for the next frame
	 */
	private void accumulateStateTransition(int senone, int indexScore,
			TrainerScore[] score, TrainerScore[] nextScore) {
		...
		Integer indexInMap = (Integer) indexMap2.get(matrix);
		int indexMatrix = indexInMap.intValue();

		// Find the corresponding buffer
		Buffer[] bufferArray = (Buffer[]) transitionsBufferPool.get(indexMatrix);
		...
		for (int i = 0; i < vector.length; i++) {
			...
			if (vector[i] != LogMath.getLogZero()) {
				...
				float beta = nextScore[indexScore].getBeta();//changed from indexNextScore
				float transitionProb = vector[i];
				float outputProb = nextScore[indexScore].getScore();//changed from indexNextScore
				float prob = transitionProb + outputProb + transitionProb + outputProb; 
				prob -= currentLogLikelihood; 
			...
			//transitionsBufferPool.put(indexMatrix, bufferArray);
		}
	//System.out.println();
	}
	...
	private void accumulateStateTransition(int indexState, SenoneHMM hmm,
		...
		//  System.out.println( transitionsPool.indexOf(matrix));
		Integer indexInMap = (Integer) indexMap2.get(matrix);
		int indexMatrix = indexInMap.intValue();
		//System.out.println(indexMatrix);
		...
		if (indexHmm == TrainerAcousticModel.ALL_MODELS) {
			...
			for (Iterator i = hmmManager.getIterator(); i.hasNext();) {
				...
				for (int j = 0; j < hmm.getOrder(); j++) {
					accumulateStateTransition(j, hmm, score[indexScore]
							.getScore());
				}
			}
		} else {
			...
			// anywhere...
			if (nextScore != null) {
				accumulateStateTransition(indexHmm, indexScore, score, nextScore);
			}
		}
	}

	/**
	 * Update the log likelihood. This method should be called for every
	 * utterance.
	 */
	public void updateLogLikelihood(TrainerScore thisScore) {
		
		likelihood = logMath.addAsLinear(likelihood, thisScore.getGamma());
	}
	...
	public void normalize() {
		...
		logNormalize2DPool(transitionsBufferPool, transitionsPool);
	...
	/**
	 * Normalize a 2D buffer pool in log scale. Typically, this is the case with
	 * the transition matrix, which also needs a mask for values that are
	 * allowed, and therefor have to be updated, or not allowed, and should be
	 * ignored.
	 * 
	 * @param pool
	 *            the buffer pool to normalize
	 * @param maskPool
	 *            pool containing a mask with zero/non-zero values.
	 */
	 ...
	 */
	private float[] copyVector(float[] in, float[] out) {
	...
	private void updateMeans() {
		...
		for (int i = 0; i < meansPool.size(); i++) {
			...
			if (buffer.wasUsed()) {
				...
				means = copyVector(meansBuffer, means);
			} else {
				//logger.info("Senone " + i + " not used.");
			}
			//meansPool.put(i, means);
		}
	}
	...
	private void updateVariances() {
		...
		for (int i = 0; i < variancePool.size(); i++) {
			...
			if (buffer.wasUsed()) {
				...
				//variancePool.put(i, varianceBuffer);
			}
		}
	}
	...
	private void updateMixtureWeights() {
		...
		for (int i = 0; i < mixtureWeightsPool.size(); i++) {
			...
			if (buffer.wasUsed()) {
				...
				//this doesn't do anything no pointers!
				mixtureWeights = copyVector(mixtureWeightsBuffer, mixtureWeights);
			}
			//mixtureWeightsPool.put(i, mixtureWeights);
		}
	}
    ...
    private void updateTransitionMatrices() {
    	...
    	for (int i = 0; i < transitionsPool.size(); i++) {
    	    float[][] matrix = (float [][])transitionsPool.get(i);
    	    Buffer[] bufferArray = (Buffer []) transitionsBufferPool.get(i);
			...
			// transitionsBufferPool.put(i, bufferArray);
			// transitionsPool.put(i, matrix);
    	    }
		}
