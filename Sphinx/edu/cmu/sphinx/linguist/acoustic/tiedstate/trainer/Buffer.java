...
class Buffer {
	...
	/**
	 * Creates a new buffer. If the values will be in log, the buffer is
	 * initialized to all <code>LogMath.getLogZero()</code>.
	 * 
	 * @param size
	 *            the number of elements in this buffer
	 * @param isLog
	 *            if true, the values in the buffer will be in log
	 */
	...
	/**
	 * Accumulates data to this buffer. Data are accumulated to a given
	 * numerator buffer and to the denominator buffer.
	 * 
	 * @param data
	 *            the data to be added
	 * @param entry
	 *            the numerator entry to be accumulated to
	 */
	 ...
	 *            the logMath to use
	 */
	void logAccumulate(float data, float denominatorData, int entry, LogMath logMath) {
		// assert numerator != null;
		// assert isLog;
		numerator[entry] = logMath.addAsLinear((float) numerator[entry], data);
		denominator = logMath.addAsLinear((float) denominator, denominatorData);
		wasUsed = true;
	}
	/**
	 * Accumulates data to this buffer. Data are accumulated to a given
	 ...
	 * 
	 * @param numeratorData
	 *            the data to be added to the numerator
	 * @param denominatorData
	 *            the data to be added to the denominator
	 */
	void accumulate(double[] numeratorData, double denominatorData, double[] featureData) {
		// assert numerator != null;
	 ...
	 * numerator buffer and to the denominator buffer.
	 * 
	 * @param logNumeratorData
	 *            the data to be added to the numerator
	 * @param logDenominatorData
	 *            the data to be added to the denominator
	 * @param logMath
	...
	void logAccumulate(float[] logNumeratorData, float logDenominatorData,
		...
		// assert isLog;
		for (int i = 0; i < numerator.length; i++) {
			numerator[i] = logMath.addAsLinear((float) numerator[i],
					logNumeratorData[i]);
		}
		denominator = logMath.addAsLinear((float) denominator,
				logDenominatorData);
		wasUsed = true;
	}
	 ...
	 * denominator, storing the result in the numerator, and setting denominator
	 * to 1.
	 */
	void normalize() {
		// I added this fp
	...
	/**
	 * Normalize the buffer in log scale. This method divides the numerator by
	 * the denominator, storing the result in the numerator, and setting
	 * denominator to log(1) = 0.
	 */
	void logNormalize() {
		// assert isLog;
		for (int i = 0; i < numerator.length; i++) {
	...
	/**
	 * Normalize the non-zero elements in a buffer in log scale. This method
	 * divides the numerator by the denominator, storing the result in the
	 * numerator, and setting denominator to log(1) = 0. A mask is used to tell
	 * whether a component should be normalized (non-zero) or not (zero).
	 * 
	 * @param mask
	 *            a vector containing zero/non-zero values.
	 */
	...
	/**
	 * Normalize the buffer. The normalization is done so that the summation of
	 * elements in the buffer is 1.
	 */
	void normalizeToSum() {
		// assert !isLog;
		float den = 0.0f;
	...
	/**
	 * Normalize the buffer in log scale. The normalization is done so that the
	 * summation of elements in the buffer is log(1) = 0. In this, we assume
	 * that if an element has a value of zero, it won't be updated.
	 * 
	 * @param logMath
	 *            the logMath to use
	 */
	 ...
	 * 
	 * @param logFloor
	 *            the floor for this buffer, in log scale
	 * 
	 * @return if true, the buffer was modified
	 */
	...
	/**
	 * Retrieves a value from this buffer. Make sure you normalize the buffer
	 * first.
	 * 
	 * @param entry
	...
	/**
	 * Retrieves a vector from this buffer. Make sure you normalize the buffer
	 * first.
	 * 
	 * @return the value
	 */
